...
static Node *addQualUniqueEdges(ParseState *pstate, Node *qual, List *ueids,
								List *ueidarrs);
/* MATCH - VLE */
static Node *arrayConcat(ParseState *pstate, Node *array, Node *elem);
/* MATCH - quals */
		...
		foreach(lp, c)
		{
			...
			for (;;)
			{
				...
				if (out)
				{
					...
					{
						pvs = arrayConcat(pstate, pvs,
										  makePathVertexExpr(pstate, vertex));
					}

					if (crel->varlen == NULL)
					{
						pes = arrayConcat(pstate, pes,
...
static Node *
arrayConcat(ParseState *pstate, Node *array, Node *elem)
{
	Oid		arrtype = exprType(array);
	Oid		elemtype = exprType(elem);

	if (!(arrtype == VERTEXARRAYOID && (elemtype == VERTEXOID ||
										elemtype == VERTEXARRAYOID)) &&
		!(arrtype == EDGEARRAYOID && (elemtype == EDGEOID ||
									  elemtype == EDGEARRAYOID)))
	{
		ereport(ERROR,
				(errcode(ERRCODE_DATATYPE_MISMATCH),
				 errmsg("expression must be a graph object or an array of it but %s, %s",
						format_type_be(arrtype), format_type_be(elemtype))));
	}
